...
for method = [1]
    ...
    figure
    a = k.*2./m.*pi.*sqrt(b.*g + h); %damping coeffience
...
%    while (du > dumax && num < itmax)
    while num < itmax
        ...
        if(mod(num,20) == 0)
            ...
            title(sprintf('Contour at Level-Set 0 at Iteration = %d ,lambda=%d,k=%d,b=%d,h=%d', num,lambda,k,b,h));
            drawnow;
        end
%         if(mod(num,20) == 0)
%             imshow(u); hold on
...
%         end
    ...
    name = strcat('i_hard_b =',num2str(b),',i=',num2str(itmax),',lambda=',num2str(lambda),',k=',num2str(k),',h=',num2str(h),'.mat');
    save(name,'E');
