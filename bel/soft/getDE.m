...
% Georgia Tech
% 2019.9.5
function [e,dE] = getDE(u,I,lambda,dx,beta,g,h)
...
%    e = g./beta.*sum(sum(sqrt(1 + beta.^2.*(DxF.^2 + DyF.^2)))) + lambda.*sum(sum(u.*(I - c1).^2)) + lambda.*sum(sum((1 - u).*(I - c2).^2)) + h.*sum(sum(h1 + h2));
