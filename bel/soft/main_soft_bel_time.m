...
for method = [1]
    ...
    figure
    a = k.*2.*sqrt(b.*g.*(pi.^2 + h)./m./n); %damping coeffience
    ...
    while (ind < tmax)
        ...
        beta = b;
        utemp = u;
        z = 4*N*beta*g/dx/dx + h;
        [e,dE] = getDE(u,I,lambda,dx,beta,g,h);
...
%             title(sprintf('Contour at Level-Set 0 at Time = %d ,lambda=%d,k=%d,b=%d,h=%d', floor(etime(t2,t1)),lambda,k,b,h));
%             drawnow;
        end
    ...
    name = strcat('t_qua_b =',num2str(b),',t=',num2str(tmax),',lambda=',num2str(lambda),',k=',num2str(k),',h=',num2str(h),'.mat');
    save(name,'E');
